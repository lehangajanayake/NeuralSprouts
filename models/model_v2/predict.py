import torch
import pandas as pd
from model_v2.dataloader import PlantDatasetV2
from model_v2.model import ModelV2
...
def predict_and_save():
    # Load test dataset
    test_dataset = PlantDatasetV2(
        RGB_dir=TEST_RGB,
        depth_dir=TEST_DEPTH,
        labels_file=TEST_CSV,
    ...
    predictions = {}
    with torch.no_grad():
        for images, _, _, image_ids in test_loader:
    ...
    if 'image_id' in df.columns:
        df['PredictedDryWeight'] = df['image_id'].map(lambda x: predictions.get(int(x), ''))
    elif 'id' in df.columns:
        df['PredictedDryWeight'] = df['id'].map(lambda x: predictions.get(int(x), ''))
    else:
