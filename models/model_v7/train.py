...
def stage1_train_rgb_classifier(cfg: TrainConfig, model: LettuceMultiBranchCNN, train_loader, val_loader, device):
    ...
    # Phase 1 improvement: Add learning rate scheduling (warmup + cosine annealing)
...
def stage2_train_rgbd_regressor(cfg: TrainConfig, model: LettuceMultiBranchCNN, train_loader, val_loader, device):
    ...
    # Phase 1 improvement: Add learning rate scheduling
    warmup_scheduler = LinearLR(optimizer, start_factor=0.1, total_iters=min(5, cfg.num_epochs_stage2))
    main_scheduler = CosineAnnealingLR(optimizer, T_max=cfg.num_epochs_stage2 - min(5, cfg.num_epochs_stage2), eta_min=1e-5)
    ...
    for epoch in range(cfg.num_epochs_stage2):
        ...
        # Step learning rate schedulers
        if epoch < min(5, cfg.num_epochs_stage2):
            warmup_scheduler.step()
        else:
            main_scheduler.step()
...
def stage3_train_fusion(cfg: TrainConfig, model: LettuceMultiBranchCNN, train_loader, val_loader, device):
    ...
    # Phase 1 improvement: Add learning rate scheduling
    warmup_scheduler = LinearLR(optimizer, start_factor=0.1, total_iters=min(5, cfg.num_epochs_stage3))
    main_scheduler = CosineAnnealingLR(optimizer, T_max=cfg.num_epochs_stage3 - min(5, cfg.num_epochs_stage3), eta_min=1e-5)
    ...
    for epoch in range(cfg.num_epochs_stage3):
        ...
        # Step learning rate schedulers
        if epoch < min(5, cfg.num_epochs_stage3):
            warmup_scheduler.step()
        else:
            main_scheduler.step()
        if stopper.step(val_mae):
...
def main():
    ...
    # Phase 1 improvement: Use best available device
    # Priority: MPS (Apple GPU) > CUDA (NVIDIA) > CPU
