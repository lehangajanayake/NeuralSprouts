...
class PreprocessConfig:
    ...
    # 20 random + 4 directional shifts = 24 total
    num_aug_per_image: int = 24
    num_random_aug: int = 20  # First 20 are random augmentations
    seed: int = 42
...
def apply_directional_shift(rgb: Image.Image, depth: Image.Image, direction: str, shift_percent: float = 0.1) -> Tuple[Image.Image, Image.Image]:
    """Apply directional shift (10% of image in one direction) and then crop.
    
    Args:
        rgb: RGB image
        depth: Depth image
        direction: 'up', 'down', 'left', or 'right'
        shift_percent: Percentage of image to shift (0.1 = 10%)
    
    Returns:
        Shifted and cropped (rgb, depth) pair
    """
    w, h = rgb.size
    crop_size = 900  # Standard crop size
    
    # Calculate shift amount (10% of image)
    shift_h = int(h * shift_percent)
    shift_w = int(w * shift_percent)
    
    # Calculate crop position based on direction
    if direction == 'up':
        # Move crop window down to see upper part
        top = shift_h
        left = (w - crop_size) // 2
    elif direction == 'down':
        # Move crop window up to see lower part
        top = h - crop_size - shift_h
        left = (w - crop_size) // 2
    elif direction == 'left':
        # Move crop window right to see left part
        top = (h - crop_size) // 2
        left = shift_w
    elif direction == 'right':
        # Move crop window left to see right part
        top = (h - crop_size) // 2
        left = w - crop_size - shift_w
    else:
        # Default to center
        top = (h - crop_size) // 2
        left = (w - crop_size) // 2
    ...
    depth = depth.crop((left, top, left + crop_size, top + crop_size))
...
def _process_one_row(args) -> List[Dict]:
    ...
    for k in range(int(cfg.num_aug_per_image)):
        ...
        if k < cfg.num_random_aug:
            # First 20: random augmentations (flips, rotations, color jitter)
            rgb_aug, depth_aug = apply_aug(rgb0, depth0, rng)
        else:
            # Last 4: directional shifts (no color changes)
            shift_index = k - cfg.num_random_aug
            directions = ['up', 'down', 'left', 'right']
            direction = directions[shift_index]
            rgb_aug, depth_aug = apply_directional_shift(rgb0, depth0, direction)
...
def main(cfg: Optional[PreprocessConfig] = None) -> None:
    ...
    cfg_dict = {
        ...
        'num_random_aug': cfg.num_random_aug,
        'seed': cfg.seed,
