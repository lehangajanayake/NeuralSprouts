...
class EvalConfig:
    ...
    normal_dir: str = '../../datasets/Training/NormalMaps'

    checkpoint: str = 'best_model_v9.pth'
    batch_size: int = 128
    seed: int = 42
    plot_path: str = 'eval_predictions_v9.png'
    errors_csv: str = 'eval_predictions_v9.csv'
...
def main(cfg: Optional[EvalConfig] = None):
    ...
    with torch.no_grad():
        for batch in loader:
            rgbn = batch['rgbn'].to(device)
            rgbd = batch['rgbd'].to(device)
            y = batch['dry_weight'].to(device)
            sample_ids = batch.get('id')
            if sample_ids is None:
                raise KeyError("Dataset must return 'id' for evaluation exports.")

            _, _, fusion_pred = model(rgbn, rgbd)
