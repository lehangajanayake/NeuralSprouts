...
class BottleneckBlock(nn.Module):
    """Residual 1×1 → 3×3 → 1×1 bottleneck with optional downsample + pooling."""
...
class SpatialAttentionModule(nn.Module):
    """CBAM-style spatial attention block."""
...
class RGBNormalRegressionBranch(nn.Module):
    """Processes RGB concatenated with normals (6 channels)."""

    def __init__(self, in_channels: int = 6, dropout: float = 0.2, drop_path_prob: float = 0.0):
        super().__init__()
    ...
    def forward(self, rgbn: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        x = self.features(rgbn)
...
class RGBDRegressionBranch(nn.Module):
    """Processes RGBD inputs (4 channels)."""
...
class LettuceNormalFusionNet(nn.Module):
    """Dual-branch CNN that fuses RGB+normal and RGBD embeddings."""

    def __init__(self, drop_path_prob: float = 0.3):
        super().__init__()
        self.rgbn_branch = RGBNormalRegressionBranch(drop_path_prob=drop_path_prob)
        self.rgbd_branch = RGBDRegressionBranch(drop_path_prob=drop_path_prob)
        self.fusion = FusionMLP(in_dim=512)

    def forward(self, rgbn: torch.Tensor, rgbd: torch.Tensor):
        rgbn_pred, rgbn_feat = self.rgbn_branch(rgbn)
        rgbd_pred, rgbd_feat = self.rgbd_branch(rgbd)
        fusion_in = torch.cat([rgbn_feat, rgbd_feat], dim=1)
        fusion_pred = self.fusion(fusion_in)
        return rgbn_pred, rgbd_pred, fusion_pred

    @torch.no_grad()
    def predict_dry_weight(self, rgbn: torch.Tensor, rgbd: torch.Tensor) -> torch.Tensor:
        self.eval()
        _, _, fusion_pred = self.forward(rgbn, rgbd)
        return fusion_pred
