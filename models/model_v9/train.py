...
from dataloader import PlantDatasetV9
from model import LettuceNormalFusionNet

RGBN_LOSS_WEIGHT = 0.2
...
@dataclass
class TrainConfig:
    train_csv: str = '../../datasets/Training/Augmented_v9/Train_aug.csv'
    rgb_dir: str = '../../datasets/Training/Augmented_v9/RGBImages'
    depth_dir: str = '../../datasets/Training/Augmented_v9/DepthImages'
    normal_dir: str = '../../datasets/Training/Augmented_v9/NormalMaps'
...
class TensorDictDataset(Dataset):
    """Dataset wrapper around preloaded tensors."""

    def __init__(self, rgb: torch.Tensor, rgbn: torch.Tensor, rgbd: torch.Tensor, dry_weight: torch.Tensor):
        self.rgb = rgb
        self.rgbn = rgbn
        self.rgbd = rgbd
...
def preload_subset_to_device(dataset, device: torch.device, label: str) -> TensorDictDataset:
    ...
    rgbn = torch.stack(rgbn_list, dim=0).to(device, non_blocking=True)
    ...
    return TensorDictDataset(rgb, rgbn, rgbd, targets)


def _build_full_dataset(cfg: TrainConfig) -> PlantDatasetV9:
    dataset = PlantDatasetV9(
        cfg.rgb_dir,
        cfg.depth_dir,
        cfg.normal_dir,
        cfg.train_csv,
...
def _split_group_indices(
    ...
    if total_groups < 2:
        raise ValueError('Need at least two unique originals to create a split.')
    ...
    if num_folds > total_groups:
        print(f"[kfold] Requested {num_folds} folds but only {total_groups} unique originals; capping at {total_groups}.")
    ...
    if num_folds > 1:
        ...
        for fold_id, val_groups in enumerate(fold_groups, 1):
            ...
            if not train_idx or not val_idx:
                raise ValueError(f'Fold {fold_id} is empty. Reduce num_folds or gather more data.')
        ...
        if not train_idx or not val_idx:
            ...
            raise ValueError(f'Group split resulted in empty train/val set. {hint}.')
        splits.append((train_idx, val_idx))
...
def train_fusion_regressor(
    ...
    best_name = f'best_model_v9{fold_suffix}.pth' if fold_suffix else 'best_model_v9.pth'
