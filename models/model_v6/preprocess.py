...
class BatchPreprocessor:
    ...
    def preprocess_dataset(self, 
        ...
        for idx, rgb_path in enumerate(rgb_files):
            ...
            # Remove 'RGB_' prefix for depth image matching
            if image_id.startswith('RGB_'):
                depth_id = image_id[len('RGB_'):]
            else:
                depth_id = image_id
            depth_base = f"Depth_{depth_id}"
            depth_path = Path(depth_dir) / f"{depth_base}.jpg"
            if not depth_path.exists():
                depth_path = Path(depth_dir) / f"{depth_base}.png"
            if not depth_path.exists():
                print(f"Warning: Depth image not found for {depth_base}, skipping...")
                continue
