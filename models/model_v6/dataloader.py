...
def _resolve_column(columns, preferred_name: str, fallback_names: List[str]) -> str:
    """Resolve a column name from preferred and fallback options (case-insensitive)."""
...
class DualBranchDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.image_id_col = _resolve_column(
            self.data.columns,
            image_id_col,
            ["image_id", "id", "ImageID", "Id"]
        )
        self.target_col = _resolve_column(
            self.data.columns,
            target_col,
            ["DryWeightShoot", "dry_weight", "target", "label"]
        )
        # Filter to entries with available preprocessed files
        available_ids = {path.stem for path in self.rgb_dir.glob("*.png")}
        if not available_ids:
            raise FileNotFoundError(f"No preprocessed RGB files found in {self.rgb_dir}")
        original_len = len(self.data)
        self.data[self.image_id_col] = self.data[self.image_id_col].astype(str)
        self.data = self.data[self.data[self.image_id_col].isin(available_ids)].reset_index(drop=True)
        filtered_len = len(self.data)
        missing = original_len - filtered_len
        if missing > 0:
            print(f"Warning: {missing} entries skipped because preprocessed files were not found.")
